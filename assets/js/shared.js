...
// ============================================================
// OFFLINE GAME LOGIC (shared across impostor/thief/spyfall)
// ============================================================
...
function handleSpyfallVote(votedPlayer) {
    ...
    if (caught) { triggerAnimation('win'); resultMsg.innerText = `براڨو! ${votedPlayer.name} هو الspy.`; }
    else { triggerAnimation('lose'); resultMsg.innerText = `غلط! الspy هرب. ${votedPlayer.name} خاطيه.`; }
    ...
    nextBtn.innerText = '🔄 عاود انده'; nextBtn.onclick = () => showScreen('setup-screen');
    showScreen('result-screen');
}
...
function handleThiefJudgement(votedPlayer) {
    ...
    if (caught) { triggerAnimation('win'); resultMsg.innerText = `الحاكم فقسها! ${votedPlayer.name} هو السارق.`; }
    else { triggerAnimation('lose'); resultMsg.innerText = `السارق هرب! ${votedPlayer.name} طلع خاطيه.`; }
...
function startThiefOffline() {
    ...
    if (namesInput.length < 3) { document.getElementById('setup-error').innerText='يلزم 3 لاعبين على الأقل: سارق، حاكم، وجلّاد.'; _sfx.error(); return; }
    document.getElementById('setup-error').innerText='';
    const shuffled = [...namesInput].sort(()=>0.5-Math.random());
    const roles = [...thiefRoles, ...Array(Math.max(0, shuffled.length - 3)).fill(null).map(() => ({ key:'witness', label:'شاهد', icon:'👁️', desc:'إنت شاهد. عاون الحاكم بالكلام وما تكشفش برشة.' }))].sort(()=>0.5-Math.random());
    players = shuffled.map((name, idx) => { const role = roles[idx]; return { name, role:role.key, roleLabel:role.label, roleIcon:role.icon, roleDesc:role.desc, eliminated:false, viewedCard:false }; });
...
function startSpyfallOffline() {
    ...
    saveSettings();
    const namesInput = Array.from(document.querySelectorAll('.player-input')).map((inp,idx)=>inp.value.trim()||`لاعب ${idx+1}`);
    if (namesInput.length < 3) { document.getElementById('setup-error').innerText='يلزم 3 لاعبين على الأقل باش تلعب ماناش هوني.'; _sfx.error(); return; }
    if (!spyfallDB.length) { document.getElementById('setup-error').innerText='قائمة البلايص مازال ما تحملتش، جرب بعد شوية.'; _sfx.error(); return; }
    document.getElementById('setup-error').innerText='';
    ...
    players = namesInput.map((name, idx) => ({ name, isSpy: idx === spyIndex, locationName: location.location_tn, locationRole: roles[idx % Math.max(1, roles.length)] || 'حريف', eliminated:false, viewedCard:false }));
    remainingTime = timerConfig * 60; currentRevealIndex = 0;
    renderSingleCard(); showScreen('reveal-screen'); _sfx.gameStart();
}
...
function _coupAlive(state = coupState) { return state.players.filter(p => p.hand.some(c => !c.lost)); }
...
function _coupPublicCard(player) { const live = player.hand.find(c=>!c.lost); return live || player.hand[0]; }
function _coupTakeFromBank(state, amount) { if (!state || !Number.isFinite(state.bankCoins)) return; state.bankCoins = Math.max(0, state.bankCoins - Math.max(0, amount || 0)); }
function _coupPayBank(state, amount) { if (!state || !Number.isFinite(state.bankCoins)) return; state.bankCoins += Math.max(0, amount || 0); }
function _coupResourceHtml(state = coupState) {
    ...
    const coinBar = typeof bank === 'number' ? `<span class="coup-coin-bar">${Array(5).fill(0).map((_,i) => `<span class="${i < coinDots ? 'coin-dot filled' : 'coin-dot'}"></span>`).join('')}</span>` : '';
...
function _coupTimerHtml(left) { return `<span>وقت الدور</span><strong>${_formatSeconds(left)}</strong>`; }
...
function _coupActionMinutes(state = coupState) { const raw = state?.actionMinutes || timerConfig || COUP_DEFAULT_ACTION_MINUTES; return Math.max(1, Math.min(10, parseInt(raw, 10) || COUP_DEFAULT_ACTION_MINUTES)); }
function _coupSetTurnDeadline(state = coupState) { if (!state) return; state.turnEndsAt = _now() + (_coupActionMinutes(state) * 60000); }
function _coupSetResponseDeadline(pending) { pending.expiresAt = _now() + COUP_RESPONSE_SECONDS * 1000; }
function _coupBlockRoleLabel(role) { const meta = coupCards[role] || coupCards.duke; return `${meta.icon} ${meta.name}`; }
function _coupBlockOptions(pending) { return (pending?.blockRoles || []).map(role => ({ role, label:_coupBlockRoleLabel(role) })); }
function _coupPendingClaimantId(pending) { return pending?.stage === 'block' ? pending.blockerId : pending?.actorId; }
function _coupPendingResponders(state = coupState, pending = state?.pending) { const claimantId = _coupPendingClaimantId(pending); return _coupAlive(state).filter(p => p.id !== claimantId); }
function _coupPassCount(state = coupState, pending = state?.pending) { const passes = new Set(pending?.passes || []); return _coupPendingResponders(state, pending).filter(p => passes.has(p.id)).length; }
function _coupAllPassed(state = coupState, pending = state?.pending) { const responders = _coupPendingResponders(state, pending); return responders.length > 0 && _coupPassCount(state, pending) >= responders.length; }
function _formatSeconds(totalSeconds) { const safe = Math.max(0, parseInt(totalSeconds, 10) || 0); const m = Math.floor(safe / 60).toString().padStart(2, '0'); const s = (safe % 60).toString().padStart(2, '0'); return `${m}:${s}`; }
function _coupCardIconHtml(card, cls = 'coup-card-avatar') { if (!card) return ''; if (card.img) return `<img class="${cls}" src="${_escapeHtml(card.img)}" alt="${_escapeHtml(card.name || '')}" loading="lazy">`; return `<span class="${cls}">${card.icon || ''}</span>`; }
function _coupCardLargeHtml(card, cls = 'coup-card-portrait') { if (!card) return ''; const src = card.img512 || card.img; if (src) return `<img class="${cls}" src="${_escapeHtml(src)}" alt="${_escapeHtml(card.name || '')}" loading="lazy">`; return `<span class="${cls}">${card.icon || ''}</span>`; }
function _coupCardLabelHtml(card, cls = 'coup-card-avatar') { return `${_coupCardIconHtml(card, cls)}<span>${_escapeHtml(card?.name || '')}</span>`; }
function _coupCardDescHtml(card) { const attack = card.attack || card.desc || ''; const defense = card.defense || ''; return `<span class="coup-card-desc-line">${_escapeHtml(attack)}</span>${defense ? `<span class="coup-card-desc-line">${_escapeHtml(defense)}</span>` : ''}`; }

function _renderCoupRoleHelp(cards = coupCards) {
    const help = document.getElementById('coup-role-help');
    if (!help) return;
    help.innerHTML = Object.values(cards).map(c => `<div class="coup-help-item"><div class="coup-help-head">${_coupCardIconHtml(c)}<strong>${_escapeHtml(c.name)}</strong></div>${_coupCardDescHtml(c)}</div>`).join('');
    const toggle = document.getElementById('coup-help-toggle');
    const chevron = document.getElementById('coup-help-chevron');
    if (toggle && !toggle.dataset.bound) {
        toggle.dataset.bound = '1';
        toggle.addEventListener('click', () => { const open = help.classList.toggle('open'); if (chevron) chevron.textContent = open ? '▲' : '▼'; _sfx.tap(); });
    }
}
...
function _showCoupModal(title, bodyHtml, setup) {
    ...
    overlay.innerHTML = `<div class="coup-modal-card" role="dialog" aria-modal="true"><button class="coup-modal-close" type="button" aria-label="close">×</button><div class="coup-modal-spark">✦</div><h3>${_escapeHtml(title)}</h3><div class="coup-modal-body">${bodyHtml}</div></div>`;
    document.body.appendChild(overlay);
...
function _wireCoupModalCountdown(root = document) {
    ...
    nodes.forEach(node => {
        const tick = () => {
            ...
            node.textContent = `${left}s`; node.classList.toggle('urgent', left <= 10);
...
function _closeCoupModal() {
    ...
    if (!overlay) return;
    overlay.classList.remove('show'); _sfx.modalClose();
    setTimeout(() => overlay.remove(), 220);
    document.querySelectorAll('.coup-action-btn').forEach(btn => { if (btn.getAttribute('aria-disabled') !== 'true') btn.disabled = false; });
}
...
function _showCoupLossAnimation(playerName, cardMeta, out = false) {
    ...
    el.innerHTML = `<div class="coup-loss-card"><div class="coup-loss-card-face">${_coupCardLargeHtml(cardMeta, 'coup-loss-icon')}<strong>${_escapeHtml(cardMeta?.name || '')}</strong></div><div class="coup-loss-cut"></div></div><div class="coup-loss-text">${_escapeHtml(playerName || '')} خسر ${_escapeHtml(cardMeta?.name || 'كارتة')}${out ? '<br><strong>خرج من الطرح.</strong>' : ''}</div>`;
    document.body.appendChild(el); _sfx.lose();
    setTimeout(() => el.remove(), 2000);
}

window.CoupUI = {
    escapeHtml: _escapeHtml, formatSeconds: _formatSeconds, renderRoleHelp: _renderCoupRoleHelp,
    showModal: _showCoupModal, closeModal: _closeCoupModal, showCardInfo: _showCoupCardInfo,
    showEvent: _showCoupEvent, showLossAnimation: _showCoupLossAnimation,
    cardIconHtml: _coupCardIconHtml, cardLabelHtml: _coupCardLabelHtml, cardLargeHtml: _coupCardLargeHtml
};
...
function renderCoupGuide(activeKey = 'goal') {
    ...
    tabs.innerHTML = coupGuideSections.map(section => `<button class="coup-guide-tab ${section.key === active.key ? 'active' : ''}" data-guide-tab="${section.key}" type="button">${section.icon}<span>${_escapeHtml(section.title)}</span></button>`).join('');
    if (active.cards) {
        content.innerHTML = `<div class="coup-guide-card-grid">${active.cards.map(type => { const card = coupCards[type]; return `<button class="coup-guide-role-card" data-guide-card="${type}" type="button">${_coupCardLargeHtml(card)}<strong>${_escapeHtml(card.name)}</strong><span>${_escapeHtml(card.attack)}</span><span>${_escapeHtml(card.defense)}</span></button>`; }).join('')}</div>`;
        content.querySelectorAll('[data-guide-card]').forEach(btn => btn.addEventListener('click', () => _showCoupCardInfo(btn.dataset.guideCard)));
    } else {
        content.innerHTML = `<div class="coup-guide-box"><div class="coup-guide-big-icon">${active.icon}</div><h3>${_escapeHtml(active.title)}</h3><p>${_escapeHtml(active.body || '')}</p><div class="coup-guide-tip-grid">${(active.tips || []).map(tip => `<div class="coup-guide-tip">${_escapeHtml(tip)}</div>`).join('')}</div></div>`;
    }
    tabs.querySelectorAll('[data-guide-tab]').forEach(btn => btn.addEventListener('click', () => { _sfx.tap(); renderCoupGuide(btn.dataset.guideTab); }));
}
...
function startCoupTurnTimer(state = coupState, myId = null) {
    ...
    tick(); coupTimerInterval = setInterval(tick, 1000);
}
...
function renderCoupScreen(state = coupState, myId = null) {
    ...
    const renderPlayerCard = (p, idx) => {
        ...
        div.querySelectorAll('.coup-card-info').forEach(btn => btn.addEventListener('click', e => { e.stopPropagation(); _showCoupCardInfo(btn.dataset.cardType); }));
        return div;
    };
    if (state.pending && myBoard) myBoard.appendChild(_renderCoupPendingBanner(state));
    const mine = orderedPlayers[0];
    if (mine && myBoard) {
        const label = document.createElement('div'); label.className = 'coup-my-deck-label';
    ...
    if (othersBoard) {
        const othersHeader = document.createElement('button'); othersHeader.type = 'button'; othersHeader.className = 'coup-other-divider';
        othersHeader.innerHTML = `<span></span><strong>كوارط اللاعبين الأخرين</strong><span></span><em>${coupOtherDecksCollapsed ? '▼' : '▲'}</em>`;
        othersHeader.addEventListener('click', () => { coupOtherDecksCollapsed = !coupOtherDecksCollapsed; renderCoupScreen(state, myId); });
        othersBoard.appendChild(othersHeader);
        const othersWrap = document.createElement('div'); othersWrap.className = 'coup-other-decks' + (coupOtherDecksCollapsed ? ' collapsed' : '');
...
function renderCoupActions(state = coupState, myId = null) {
    const panel = document.getElementById('coup-action-panel'); panel.innerHTML = '';
    const alive = _coupAlive(state);
    if (alive.length <= 1) {
        const btn = document.createElement('button'); btn.className = 'primary-btn'; btn.innerText = '🔄 عاود انده';
        btn.onclick = () => showScreen('setup-screen');
        panel.appendChild(btn); return;
    }
    ...
    if (state.pending) { panel.innerHTML = `<div class="coup-panel-card live">${_escapeHtml(state.log)}</div>`; return; }
    const current = state.players[state.turnIndex];
    if (!isTurn) { panel.innerHTML = `<div class="coup-panel-card">استنى دورك. الدور توّة على ${_escapeHtml(current?.name || '')}.</div>`; }
    ...
    panel.querySelectorAll('.coup-action-info').forEach(info => info.addEventListener('click', e => {
        e.stopPropagation(); const meta = coupActionHelp[info.dataset.actionInfo];
...
function _renderCoupPendingBanner(state = coupState) {
    ...
    const wrap = document.createElement('div'); wrap.className = 'coup-pending-banner';
    const responders = _coupPendingResponders(state, p);
    const challengeButtons = (p.claim || isBlockStage) ? responders.map(player => `<button class="coup-target-btn danger-action" data-banner-challenge="${player.id}">${_escapeHtml(player.name)}: تكذب!</button>`).join('') : '';
    const passButtons = responders.filter(player => !(p.passes || []).includes(player.id)).map(player => `<button class="coup-target-btn quiet-action" data-banner-pass="${player.id}">${_escapeHtml(player.name)}: ما عندي حتى اعتراض</button>`).join('');
    const blockButtons = !isBlockStage && p.blockable
        ? (p.action === 'foreignAid' ? responders : responders.filter(x => x.id === p.targetId)).map(player => _coupBlockOptions(p).map(opt => `<button class="coup-target-btn" data-banner-block="${player.id}" data-block-role="${opt.role}">${_escapeHtml(player.name)}: نسكّرها ب${opt.label}</button>`).join('')).join('') : '';
    wrap.innerHTML = `<div class="coup-pending-title">قرار مباشر</div><strong>${_escapeHtml(state.log || '')}</strong><p>${isBlockStage ? `${_escapeHtml(blocker?.name || '')} قال يسكّر. أي لاعب ينجم يقول تكذب.` : `${target ? `${_escapeHtml(target.name)} مستهدف. ` : ''}${p.claim ? 'أي لاعب ينجم يقول تكذب أو ما عنديش اعتراض.' : 'أي لاعب ينجم يسكّرها أو يقول ما عندو اعتراض.'}`}</p>${_coupPendingTimerHtml(p)}<div class="coup-pass-progress">${_coupPassCount(state, p)}/${responders.length} قالو ما عندهم حتى اعتراض</div><div class="coup-pending-actions">${challengeButtons}${blockButtons}${passButtons}</div>`;
    wrap.querySelectorAll('[data-banner-challenge]').forEach(btn => btn.addEventListener('click', () => { isBlockStage ? coupChallengeBlock(btn.dataset.bannerChallenge) : coupChallenge(btn.dataset.bannerChallenge); }));
...
function coupChooseAction(action) {
    ...
    _showCoupModal(actionName, `<p>باش تعمل <strong>${_escapeHtml(actionName)}</strong>. كان فيها تبلعيط، اللاعبين ينجموا يقولو "تكذب!".</p><button class="primary-btn" id="coup-confirm-action">كمّل</button>`, overlay => {
        overlay.querySelector('#coup-confirm-action')?.addEventListener('click', () => { _closeCoupModal(); coupStartPending(action, null); });
    });
}
...
function coupPickTarget(action) {
    ...
    _showCoupModal(action === 'steal' ? 'اختار شكون تسرق' : 'اختار شكون تضرب', `<p>${action === 'steal' ? 'الرايس يسرق حتى زوز فلوس من لاعب.' : action === 'assassinate' ? 'حفار القبور يحتاج هدف واضح.' : 'Coup ضربة مباشرة وما تتسكرش.'}</p><div class="coup-target-grid">${targets.map(p => `<button class="coup-target-btn" data-target-id="${p.id}">${_escapeHtml(p.name)}</button>`).join('')}</div>`, overlay => {
        overlay.querySelectorAll('[data-target-id]').forEach(btn => btn.addEventListener('click', () => { _closeCoupModal(); coupStartPending(action, btn.dataset.targetId); }));
    });
}
...
function _coupStartResponseTimer(onExpire) {
    ...
    const tick = () => {
        ...
        if (_now() >= p.expiresAt) { clearInterval(coupResponseInterval); coupResponseInterval = null; _closeCoupModal(); onExpire?.(); }
    };
    tick(); coupResponseInterval = setInterval(tick, 500);
}
...
function renderCoupChallengePanel() {
    ...
    const challengeButtons = (p.claim || isBlockStage) ? challengers.map(c => `<button class="coup-target-btn danger-action" data-challenge-id="${c.id}">${_escapeHtml(c.name)}: تكذب!</button>`).join('') : '';
    const passButtons = _coupPendingResponders(coupState, p).filter(c => !(p.passes || []).includes(c.id)).map(c => `<button class="coup-target-btn quiet-action" data-pass-id="${c.id}">${_escapeHtml(c.name)}: ما عندي حتى اعتراض</button>`).join('');
    const blockButtons = p.blockable ? blockers.map(c => _coupBlockOptions(p).map(opt => `<button class="coup-target-btn" data-block-id="${c.id}" data-block-role="${opt.role}">${_escapeHtml(c.name)}: نسكّرها ب${opt.label}</button>`).join('')).join('') : '';
    ...
    _showCoupModal('بوّع ولا صحيح؟', `<p>${_escapeHtml(coupState.log)}</p>${targetLine}${_coupPendingTimerHtml(p)}<div class="coup-pass-progress">${_coupPassCount(coupState, p)}/${_coupPendingResponders(coupState, p).length} قالو ما عندهم حتى اعتراض</div><div class="coup-target-grid">${challengeButtons}${blockButtons}${passButtons}</div>`, overlay => {
...
function coupMarkInfluenceLost(player, card) {
    ...
    if (out && player.coins > 0) { _coupPayBank(coupState, player.coins); player.coins = 0; }
...
function coupLoseInfluence(playerId, after = null, reason = '') {
    ...
    const choose = ({card}) => { coupMarkInfluenceLost(p, card); _closeCoupModal(); after?.(); };
    if (live.length === 1) { choose(live[0]); return; }
    _showCoupModal(`${p.name}، اختار كارتة تخسرها`, `<p>${_escapeHtml(reason || 'القانون يقول إنت تختار شنية الكارتة الي تكشفها وتخسرها.')}</p><div class="coup-target-grid">${live.map(({card, index}) => { const meta = coupCards[card.type] || coupCards.duke; return `<button class="coup-target-btn" data-lose-card="${index}">${_coupCardLabelHtml(meta)}</button>`; }).join('')}</div>`, overlay => {
        overlay.querySelectorAll('[data-lose-card]').forEach(btn => btn.addEventListener('click', () => { const picked = live.find(x => String(x.index) === btn.dataset.loseCard); if (picked) choose(picked); }));
    });
}
...
function coupStartExchangeChoice(actor, after = null) {
    ...
    const render = overlay => {
        ...
        list.innerHTML = pool.map((item, idx) => { const meta = coupCards[item.type] || coupCards.duke; return `<button class="coup-target-btn ${selected.has(idx) ? 'selected' : ''}" data-exchange-pick="${idx}">${_coupCardLabelHtml(meta)}<small>${item.drawn ? 'من الدكّة' : 'من كوارطك'}</small></button>`; }).join('');
        count.textContent = `${selected.size}/${keep}`;
        overlay.querySelector('#coup-confirm-exchange').disabled = selected.size !== keep;
        list.querySelectorAll('[data-exchange-pick]').forEach(btn => btn.addEventListener('click', () => { const idx = parseInt(btn.dataset.exchangePick, 10); if (selected.has(idx)) selected.delete(idx); else if (selected.size < keep) selected.add(idx); render(overlay); }));
    };
    _showCoupModal('بدّل مع الدكّة', `<p>اختار ${keep} كارتة باش تخليها عندك. الباقي يرجع للدكّة ويتخلط.</p><div class="coup-exchange-count"></div><div class="coup-target-grid coup-exchange-grid"></div><button id="coup-confirm-exchange" class="primary-btn" type="button">ثبّت الاختيار</button>`, overlay => {
        render(overlay);
...
function coupChallenge(challengerId) {
    ...
    if (hasIt) {
        ...
        coupLoseInfluence(challengerId, () => { if (originalPending.blockable) _coupOpenBlockWindowAfterChallenge(originalPending, actor); else coupResolveAction(originalPending.action, originalPending.targetId); }, 'طلعت غالط في التكذيب. اختار كارتة تخسرها.');
    } else {
...
function coupPassPending(playerId) {
    const p = coupState.pending; if (!p) return;
    const claimantId = _coupPendingClaimantId(p); if (playerId === claimantId) return;
    p.passes = Array.from(new Set([...(p.passes || []), playerId]));
    if (_coupAllPassed(coupState, p)) { _closeCoupModal(); if (p.stage === 'block') coupAcceptBlock(); else coupResolveAction(p.action, p.targetId); }
    else { renderCoupScreen(); if (p.stage === 'block') renderCoupBlockChallengePanel(); else renderCoupChallengePanel(); }
}
...
function renderCoupBlockChallengePanel() {
    ...
    const challengeButtons = _coupPendingResponders(coupState, p).map(c => `<button class="coup-target-btn danger-action" data-challenge-block-id="${c.id}">${_escapeHtml(c.name)}: تكذب على البلوك!</button>`).join('');
    const passButtons = _coupPendingResponders(coupState, p).filter(c => !(p.passes || []).includes(c.id)).map(c => `<button class="coup-target-btn quiet-action" data-pass-id="${c.id}">${_escapeHtml(c.name)}: ما عندي حتى اعتراض</button>`).join('');
    _showCoupModal('سكّرها، أما صحيح؟', `<p>${_escapeHtml(coupState.log)}</p><p class="coup-decision-hint">أي لاعب غير ${_escapeHtml(blocker?.name || '')} ينجم يتهم البلوك بالتبلعيط، ولا يعمل ما عندي حتى اعتراض.</p>${_coupPendingTimerHtml(p)}<div class="coup-pass-progress">${_coupPassCount(coupState, p)}/${_coupPendingResponders(coupState, p).length} قالو ما عندهم حتى اعتراض</div><div class="coup-target-grid">${challengeButtons}${passButtons}</div>`, overlay => {
...
function coupAcceptBlock() {
    const p = coupState.pending; const blocker = coupState.players.find(x=>x.id===p?.blockerId);
    coupState.log = `${blocker?.name || ''} سكّر الأكشن. تعدّت بسلام.`;
    coupState.pending = null; _coupNextTurn();
    _showCoupEvent(coupState.log, 'good'); _sfx.notify(); renderCoupScreen();
}
...
function coupResolveAction(action, targetId) {
    ...
    if (action === 'exchange') {
        ...
        return coupStartExchangeChoice(actor, () => { coupState.log = `${actor.name} بدّل كوارطو مع الدكّة. السمسار خدم خدمتو.`; _showCoupEvent(coupState.log, 'good'); _coupNextTurn(); renderCoupScreen(); });
    }
    if (action === 'steal' && target) { const amount = Math.min(2, target.coins || 0); target.coins -= amount; actor.coins += amount; coupState.log = amount > 0 ? `${actor.name} سرق ${amount} فلوس من ${target.name}. الرايس دخل للمرسى.` : `${actor.name} حاول يسرق ${target.name} أما ما لقى شي.`; }
    if (action === 'assassinate' && target) { coupState.log = `${target.name} تضرّب من حفار القبور. ${target.name} يختار كارتة يخسرها.`; _showCoupEvent(coupState.log, 'bad'); return coupLoseInfluence(target.id, () => { _coupNextTurn(); renderCoupScreen(); }, 'تضرّبت من حفار القبور. اختار شنية الكارتة الي تخسرها.'); }
    if (action === 'coup' && target) { actor.coins -= 7; _coupPayBank(coupState, 7); coupState.log = `${actor.name} عمل Coup على ${target.name}. ${target.name} يختار كارتة يخسرها.`; _showCoupEvent(coupState.log, 'bad'); return coupLoseInfluence(target.id, () => { _coupNextTurn(); renderCoupScreen(); }, 'تضرّبت بCoup. اختار شنية الكارتة الي تخسرها.'); }
